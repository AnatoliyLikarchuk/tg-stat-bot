...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Бот працює у групах логістики та автоматично розпізнає повідомлення "
        "про початок і завершення збору, виїзд, завершення маршруту та проблеми "
        "доставки. Розпізнані події зберігаються у Google Sheets.\n\n"
        "📏 Облік пробігу\n"
        "Напиши у групі прізвище водія та пробіг, наприклад:\n"
        "Косич 120 км\n"
        "Якщо запис успішний, бот поставить реакцію 🏆. Повторне повідомлення "
        "за той самий день оновить значення, а не створить дубль. Пробіг "
        "приймається лише для чинних водіїв.\n\n"
        "👥 Керування водіями\n"
        "• «Додати водія» — вибери місто, введи канонічне прізвище українською "
        "та норму витрати пального. Формули створяться автоматично.\n"
        "• «Перемістити до звільнених» — рядок водія разом з історією та "
        "формулами перейде до розділу «Звільнені». Після цього новий пробіг "
        "для нього не приймається.\n"
        "• «Повернути до чинних» — повертає водія до вибраного міста. Якщо "
        "архівний блок міста спорожнів, він видаляється автоматично, а загальний "
        "розділ «Звільнені» залишається.\n"
        "• «↩️ Скасувати» — безпечно припиняє поточну дію. Після переміщення "
        "також доступне швидке повернення водія.\n\n"
        "🔤 Аліаси\n"
        "Якщо водій пише прізвище іншим варіантом, наприклад російською, "
        "додай точний аліас до його канонічного українського прізвища. Аліаси "
        "можна додавати й видаляти через меню.\n\n"
        "📱 Кнопки особистого меню\n"
        "• Статистика сьогодні — події за поточний день.\n"
        "• За тиждень — статистика за останні 7 днів.\n"
        "• Активні маршрути — маршрути, які ще не завершено.\n"
        "• Кілометраж за тиждень — пробіг водіїв за 7 днів.\n"
        "• Заповнити формули — аварійне відновлення відсутніх формул у таблиці; "
        "у звичайній роботі натискати не потрібно."
    )
