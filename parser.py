...
class MessageParser:
    ...
    PATTERNS = {
        ...
        # "Косич проехав 120 км", "Горобець (Галунько) 133 км" (за чужой маршрут).
        # Граница слова после "км" отсекает "километров".
        # Допускает 0-2 слов между именем и числом (под "за сегодня", "проехав", "сегодня").
        # Опциональная вставка в скобках (имя замещаемого/чей маршрут) — пробег идёт первому имени.
        "mileage": r"([А-ЯІЇЄҐЁ][а-яіїєґё']+)[\s,\-—–]*(?:\([^)]*\)[\s,\-—–]*)?(?:[а-яіїєґё]+\s+){0,2}(\d{1,4})\s*км\b",
    }
...
if __name__ == "__main__":
    ...
    test_messages = [
        ...
        # Пробег с заместителем в скобках (за чужой маршрут)
        "Горобець (Галунько) 133 км",
    ]
