...
class MessageParser:
    ...
    PATTERNS = {
        ...
        "route_complete": r"мар[шщ]рут\s+(?:заверш|закінч|закри)|(?:все\s+)?развез|(?:всё\s+)?развёз|мар[шщ]рут\s+закінчи|закінчив|закончил|завершив|мар[шщ]рут\s*\d*\s*закрит[оа]?|закрыт",
